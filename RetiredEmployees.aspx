<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.master" AutoEventWireup="true" CodeBehind="RetiredEmployees.aspx.cs" Inherits="PSIC.RetiredEmployees" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    &nbsp;
    <script type="text/javascript">
        $(document).ready(function () {
            $('.heading h3').html('Retire Employees');
            ...
            function Search() {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $.each(jData, function (i, item) {
                            out = out + '<tr><td> ' + item.sno + '</td> <td> ' + item.EmpNo + '</td>  <td> ' + item.Full_Name + '</td>  <td> ' + item.FatherName + '</td> <td> ' + item.CNIC + '</td> <td> ' + item.DepartmentName + '</td> <td> ' + item.Designation + '</td> <td>    <input class="clsSelectedEmployees" type="checkbox" tag="' + item.User_ID + '"/>     </td> </tr>';
                        });
            ...
            $('#btnRetire').bind('click', function () {
                var empids = "";
                $('.clsSelectedEmployees').each(function (i, item) {
                    if ( $(this).attr('checked') == 'checked') {
                        empids = empids + $(this).attr('tag') + ',';
                    }
                });
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    url: "RetiredEmployees.aspx/RetireEmployees",
                    data: "{ 'Ids' : '" + empids + "'}",
                    success: function (response) {
        ...
        $('body').on({
            click: function () {
                if ($('#chkAllRetire').attr('checked') == 'checked') {
                    $('.clsSelectedEmployees').each(function (i, item) {
                ...
                <div class="content">
                    ...
                    <form class="form-horizontal" action="#">
                                ...
                                <table id="tbSearchedEmployees" class="responsive table table-striped table-bordered table-condensed">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>
                                                <input id="chkAllRetire" type="checkbox" />
                                            </th>
                        ...
                        <div class="form-row row-fluid">
                            <div class="span6">
                                <label class="form-label span4" for="normal"></label>
                                <button type="button" class="btn btn-primary" id="btnRetire">Retire Selected Employees</button>
