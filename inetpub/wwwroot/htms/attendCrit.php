...
function changeReportOrder(itm) {
	var ro = document.getElementById ('rptOrder');
	if (itm.value != "") // Specific Employee
		ro.value = "2"; 
	else
		ro.value = "1";
}
...
<?php 
if (strpos($_POST['flg'], 'Report') !== false && isset($_POST['showReport']) && $_POST['showReport'] != '') { 
	$_SESSION['rptFormat'] = $_POST['rptFormat'];
	$_SESSION['deptID'] = $_POST['deptID'];
	$_SESSION['Emp_Id'] = $_POST['Emp_Id'];
	$_SESSION['aType'] = $_POST['aType'];
	$_SESSION['rptOrder'] = $_POST['rptOrder'];
	$_SESSION['fromDt'] = $_POST['fromDt'];
	$_SESSION['toDt'] = $_POST['toDt'];
	
	if ($_POST['rptOrder'] == 2) // Employee wise
		$_POST['flg'] = 'Emp Wise Attendance Report';
		
	$_SESSION['flg'] = $_POST['flg'];

	header ("Location: rptViewer.php");
}

$pageItem = 'Attendance Employees';
$tabTitle = 'Please enter criteria for ' . $pageItem . ' Report:';
$pageFor = 'Report';
?>
